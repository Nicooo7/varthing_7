...
def tableau_de_bord(request):
	...
	def data_propositions_proches_des_miennes (utilisateur):
		...
		#selectionner les triplets correspondant à mes propositions
		selection = []
		liste_de_mes_propositions = Proposition.objects.filter(soutien__user = utilisateur)
		print ("liste de mes propositions________", liste_de_mes_propositions)	
		for triplet in liste_proximites:
			proposition1 = triplet[0]
			if proposition1 in liste_de_mes_propositions:
	...
	def les_x_revendications_les_plus_proches_des_miennes(n):
		...
		for triplet in selection:
			if triplet[1] in liste_de_mes_propositions:
	...
	def creation_graph_mes_propositions():
		...
		enregistrer_les_datas(data = graph, nom = "graph_mes_propositions")
	...
	def creer_les_datas(utilisateur):
		...
		class Datas:
			def __init__ (self):
				...
				self.competence = Competence.objects.filter(soutien__user = utilisateur, soutien__lien = 'SO')
	...
	creation_graph_mes_propositions()
